...
fetch("data/products.json")
  ...
  .then((products) => {
    ...
    const gallery = p.photos && p.photos.length > 0
    ...
    document.querySelector("#product-detail").innerHTML = `<section class="product-detail"><div class="detail-gallery">${gallery}</div><div class="detail-copy"><p class="eyebrow">${p.category} / ${p.code}</p><h1>${p.name}</h1><p class="detail-description">${p.description}</p><dl><div><dt>Fabric</dt><dd>${p.fabric}</dd></div><div><dt>Colour</dt><dd>${p.color}</dd></div><div><dt>${p.sizes === "One size" ? "Size" : "Sizes"}</dt><dd>${p.sizes}</dd></div><div><dt>Availability</dt><dd>${p.availability}</dd></div></dl><a class="button button-dark order-button" href="${messageUrl(p)}" target="_blank" rel="noreferrer">Enquire on WhatsApp <span>↗</span></a><p class="order-note">We'll confirm the price and availability directly on WhatsApp.</p></div></section>`;
  })
  .catch(() => {
    document.querySelector("#product-detail").textContent = "This product could not be loaded. Please return to the collection.";
  });
